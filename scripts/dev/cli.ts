...
async function main(): Promise<number> {
  ...
  switch (command) {
    ...
    default:
      console.error(
        "usage: dev [up|down|status|restart|canary|logs|doctor] [--json] [--force] [--rotate] [--strict] [--sandbox local|sprites|smolmachines|e2b|porter|agent37|superserve|auto] [--no-slack] [--no-watch] [--org id] [--fix]",
      );
      return EXIT.usage;
  }
}
